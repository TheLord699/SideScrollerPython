import pygame as pg
import os
import math
import numpy as np

from background import Background
from game import Environment
from player import Player
from entities import Entities
from map import Map
from ui import UI 
from particles import Particles
from memory_debugger import MemoryDebugger

class LightSource:
	def __init__(self, game):
		self.game = game
		self.lights = []
		self.ambient_light = 50
		self.light_surface = None
		self.light_mask_cache = {}
		self.resize_light_surface()

	def resize_light_surface(self):
		self.light_surface = pg.Surface(
			(self.game.screen_width, self.game.screen_height), pg.SRCALPHA
		)

	def add_light(self, x, y, radius, intensity, color=(255, 255, 255)):
		self.lights.append({
			'x': x,
			'y': y,
			'radius': radius,
			'intensity': intensity,
			'color': color
		})

	def clear_lights(self):
		self.lights = []

	def get_light_mask(self, radius, color, intensity):
		key = (radius, color, round(intensity, 2))
		if key in self.light_mask_cache:
			return self.light_mask_cache[key]

		size = radius * 2
		surface = pg.Surface((size, size), pg.SRCALPHA)
		arr = np.zeros((size, size, 4), dtype=np.uint8)

		for y in range(size):
			for x in range(size):
				dx = x - radius
				dy = y - radius
				dist = math.sqrt(dx * dx + dy * dy)
				if dist <= radius:
					falloff = 1 - (dist / radius)
					brightness = max(0, min(1, falloff * intensity))
					arr[y, x, 0] = int(color[0] * brightness)
					arr[y, x, 1] = int(color[1] * brightness)
					arr[y, x, 2] = int(color[2] * brightness)
					arr[y, x, 3] = int(255 * brightness)

		pg.surfarray.blit_array(surface, arr[:, :, :3])
		alpha = pg.surfarray.pixels_alpha(surface)
		alpha[:, :] = arr[:, :, 3]
		del alpha
		self.light_mask_cache[key] = surface
		return surface

	def render(self):
		self.light_surface.fill((0, 0, 0, 255))

		for light in self.lights:
			screen_x = int(light['x'] - self.game.player.cam_x)
			screen_y = int(light['y'] - self.game.player.cam_y)
			radius = light['radius']
			mask = self.get_light_mask(radius, light['color'], light['intensity'])
			self.light_surface.blit(mask, (screen_x - radius, screen_y - radius), special_flags=pg.BLEND_ADD)

	def apply_lighting(self, screen):
		temp_surface = pg.Surface((self.game.screen_width, self.game.screen_height)).convert()
		temp_surface.fill((self.ambient_light, self.ambient_light, self.ambient_light))
		temp_surface.blit(self.light_surface, (0, 0), special_flags=pg.BLEND_ADD)
		screen.blit(temp_surface, (0, 0), special_flags=pg.BLEND_MULT)

class Game:
	def __init__(self):
		pg.init()
		self.clock = pg.time.Clock()

		os.chdir(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
		version = "0.0.0"
		icon = pg.image.load("assets/sprites/enemy/bug.png")

		self.screen_width, self.screen_height = 800, 600
		self.screen = pg.display.set_mode(
			(self.screen_width, self.screen_height),
			pg.DOUBLEBUF | pg.HWSURFACE | pg.RESIZABLE | pg.SCALED
		)

		pg.display.set_caption(f"SideScroller {version}")
		pg.display.set_icon(icon)

		self.init_game_objects()
		self.game_loop()

	def init_game_objects(self):
		self.ui = UI(self)
		self.environment = Environment(self)
		self.map = Map(self)
		self.player = Player(self)
		self.entities = Entities(self)
		self.background = Background(self)
		self.particles = Particles(self)
		self.memory_debugger = MemoryDebugger(self)
		self.lighting = LightSource(self)
		self.active_lights = []

	def render_fps(self):
		fps = self.clock.get_fps()
		fps_text = self.environment.fonts["pixel"].render(f"FPS: {round(fps)}", True, (255, 255, 255))
		self.screen.blit(fps_text, (self.screen_width - 120, 10))

	def update(self):
		self.environment.update()
		self.background.update()
		self.map.update()
		self.entities.update()
		self.player.update()
		self.ui.update()
		self.particles.update()

		self.lighting.clear_lights()

		self.lighting.add_light(
			self.player.x + self.player.hitbox_width / 2,
			self.player.y + self.player.hitbox_height / 2,
			radius=200,
			intensity=1.0,
			color=(255, 255, 200)
		)

		for light in self.active_lights:
			self.lighting.add_light(*light)

		self.lighting.render()

	def game_loop(self):
		running = True
		while running:
			self.events = pg.event.get()
			for event in self.events:
				if event.type == pg.QUIT:
					running = False

				if self.memory_debugger.show_memory_info:
					self.memory_debugger.handle_mouse_event(event)
					self.memory_debugger.handle_terminal_input(event)

					if event.type == pg.KEYDOWN:
						if event.key in (pg.K_UP, pg.K_PAGEUP):
							self.memory_debugger.handle_scroll("up")

						elif event.key in (pg.K_DOWN, pg.K_PAGEDOWN):
							self.memory_debugger.handle_scroll("down")

				else:
					if event.type == pg.KEYDOWN:
						if event.key == pg.K_h:
							self.player.take_damage(0.1)

						elif event.key == pg.K_j:
							self.player.current_health += 0.5

						elif event.key == pg.K_k:
							self.player.vel_y = -15

						elif event.key == pg.K_l:
							self.player.max_health += 1
							self.player.current_health = self.player.max_health

						elif event.key == pg.K_b:
							self.environment.menu = "main"

						elif event.key == pg.K_m:
							self.memory_debugger.toggle()

						elif event.key == pg.K_n:
							new_light = (
								self.player.x + self.player.hitbox_width / 2,
								self.player.y + self.player.hitbox_height / 2,
								200,
								1.0,
								(255, 255, 200)
							)
							self.active_lights.append(new_light)

			self.screen.fill((0, 0, 0))

			self.update()
			self.lighting.apply_lighting(self.screen)

			self.memory_debugger.render()
			self.render_fps()

			pg.display.flip()
			self.clock.tick(self.environment.fps)

		pg.quit()

if __name__ == "__main__":
	Game()
