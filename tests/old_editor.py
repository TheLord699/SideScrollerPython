...
VISUAL_SCALE = 2  # Makes tiles appear as if they were 16x16 scaled up
BASE_TILE_SIZE = 16  # The reference size we want everything to visually match
...
def load_tilesheets(tilesheets):
    ...
    for sheet in tilesheets:
        try:
            ...
            # Calculate scale factor to make tiles appear as if they were BASE_TILE_SIZE
            ...
            for y in range(0, sheet_height - tile_size + 1, tile_size):
                for x in range(0, sheet_width - tile_size + 1, tile_size):
                    ...
                    # Apply visual scaling while preserving actual dimensions
            ...
            all_surfaces.append({
                ...
                "tile_size": tile_size,
                "visual_size": int(tile_size * scale_factor),  # The displayed size
...
def save_map(folder, tiles, tilesheets):
    ...
    save_individual_tiles(tiles, tilesheets, folder)
...
def draw_grid(panel_width):
    # Get visual size from first tilesheet (assuming all have same visual size)
    visual_size = all_tile_surfaces[0]["visual_size"] if all_tile_surfaces else BASE_TILE_SIZE * VISUAL_SCALE
    
    for x in range(0, screen.get_width() - panel_width, visual_size):
        pg.draw.line(screen, (50, 50, 50), (x, 0), (x, screen.get_height()))
    for y in range(0, screen.get_height(), visual_size):
        pg.draw.line(screen, (50, 50, 50), (0, y), (screen.get_width() - panel_width, y))
    
    if precision_mode:
        for x in range(0, screen.get_width() - panel_width, visual_size // 2):
            color = (70, 70, 70) if x % visual_size == 0 else (40, 40, 40)
            pg.draw.line(screen, color, (x, 0), (x, screen.get_height()))
        for y in range(0, screen.get_height(), visual_size // 2):
            color = (70, 70, 70) if y % visual_size == 0 else (40, 40, 40)
            pg.draw.line(screen, color, (0, y), (screen.get_width() - panel_width, y))
...
def draw_tiles(tiles, all_tile_surfaces, camera_x, camera_y, show_layers=False, current_layer=0, layer_mode=False, panel_width=256):
    ...
    for tile in sorted(tiles, key=lambda t: t["layer"]):
        # Use visual size for positioning
        visual_size = all_tile_surfaces[tile.get("tilesheet", 0)]["visual_size"]
        tx = tile["x"] * visual_size - camera_x
        ty = tile["y"] * visual_size - camera_y
        
        if tile.get("type") == "entity":
            # Draw entity placeholder using visual size
...
def get_tile_at(mx, my, tiles, all_tile_surfaces, camera_x, camera_y, current_layer=None, panel_width=256):
    ...
    if precision_mode:
        gx = (mx + camera_x) / visual_size
        gy = (my + camera_y) / visual_size
        rounded_gx = round(gx * 2) / 2
        rounded_gy = round(gy * 2) / 2
        for t in tiles:
            if (abs(t["x"] - rounded_gx) < 0.01 and abs(t["y"] - rounded_gy) < 0.01 and 
...
def draw_tile_selector(all_tile_surfaces, selected_tile_info, scroll_y, panel_width):
    ...
    # Draw tabs for each tilesheet
    tab_height = 30
    for i in range(len(all_tile_surfaces)):
        tab_width = panel_width // max(1, len(all_tile_surfaces))
        tab_rect = pg.Rect(panel_x + i * tab_width, panel_y, tab_width, tab_height)
    ...
    # Draw tiles from current tilesheet
    ...
    for i, tile in enumerate(current_sheet["surfaces"]):
        ...
        y = panel_y + row * visual_size + tab_height
...
def draw_entity_selector(entity_data, selected_entity_type, selected_entity, scroll_y, panel_width):
    ...
    # Draw tabs for each entity type
    tab_height = 30
    ...
    if entity_data and selected_entity_type in entity_data:
        ...
        for i, (name, data) in enumerate(entities.items()):
            y = panel_y + tab_height + i * item_height
...
def draw_entity_inspector(entity, entity_data, panel_width):
    ...
    if entity['entity_type'] in entity_data and entity['entity_name'] in entity_data[entity['entity_type']]:
        ...
        for attr, value in entity_info.items():
            if attr in ["tile_sheet", "index"]:  # Skip these as they're handled separately
                continue
...
def handle_animation_editor_events(event, tile, selected_tile_info, all_tile_surfaces):
    if event.type == pg.KEYDOWN:
        if event.key == pg.K_RETURN:
            ...
            return True  # Cancel without saving
            ...
            if "tile" in selected_tile_info and selected_tile_info["tile"] is not None:
                # Only add if the tile is from the same tilesheet
...
while running:
    ...
    # Get visual size for calculations
    visual_size = all_tile_surfaces[0]["visual_size"] if all_tile_surfaces else BASE_TILE_SIZE * VISUAL_SCALE
    ...
    if not editing_animation:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                if show_entity_panel and mx >= screen.get_width() - current_entity_panel_width:
                    ...
                    if all_tile_surfaces and selected_tile_info["tilesheet"] < len(all_tile_surfaces):
                        tiles_per_row = max(1, (current_panel_width - 10) // visual_size)
                        rows_needed = (len(all_tile_surfaces[selected_tile_info["tilesheet"]]["surfaces"]) + tiles_per_row - 1) // tiles_per_row
                        max_scroll = max(0, (rows_needed + 1) * visual_size - screen.get_height() + 10)
                ...
                if show_version_menu:
                    ...
                    if save_rect and save_rect.collidepoint(mx, my):
                        ...
                        # Save the entity data back to file
                        try:
                        ...
                        # Check if clicking on an attribute to edit
                        panel_x = 10
                        panel_y = 10 + 30 * 4  # Skip header
                    ...
                    if (resize_handle_left <= mx <= resize_handle_left + 10 and 
                        ...
                        if event.button == 1:
                            if placing_entity and selected_entity:
                                if precision_mode:
                                    gx = round((mx + camera_x) / visual_size * 2) / 2
                                    gy = round((my + camera_y) / visual_size * 2) / 2
                                else:
                                    gx = (mx + camera_x) // visual_size
                                    gy = (my + camera_y) // visual_size
                ...
                if inspecting_entity and editing_attribute:
                ...
                elif event.key == pg.K_t:  # Toggle entity panel
                    ...
                    if event.key == pg.K_p:
                        ...
                        # Check if clicking on an entity to inspect
                        for tile in reversed(tiles):  # Check from top to bottom
                            if tile.get("type") == "entity":
                                tx = tile["x"] * visual_size - camera_x
                                ty = tile["y"] * visual_size - camera_y
                                entity_rect = pg.Rect(tx, ty, visual_size, visual_size)
                        ...
                        if tile:
                            if "animation" in tile:
                                ...
                                # Create new animation with current tile as first frame
                                tile["animation"] = {
                                    "frames": [tile["id"]],
    ...
    if not editing_animation:
        ...
        if mx < screen.get_width() - current_panel_width and all_tile_surfaces and selected_tile_info["tilesheet"] < len(all_tile_surfaces):
            if selected_tile_info["tile"] < len(all_tile_surfaces[selected_tile_info["tilesheet"]]["surfaces"]):
                ...
                if precision_mode:
                    px = round((mx + camera_x) / visual_size * 2) / 2 * visual_size - camera_x
                    py = round((my + camera_y) / visual_size * 2) / 2 * visual_size - camera_y
                else:
                    px = ((mx + camera_x) // visual_size) * visual_size - camera_x
                    py = ((my + camera_y) // visual_size) * visual_size - camera_y
        ...
        info = f"Tile: {selected_tile_info['tile']} (Sheet {selected_tile_info['tilesheet']+1})  Rot: {rotation}°  Layer: {current_layer}  Hitbox: {placing_hitbox}  Camera: ({camera_x}, {camera_y})  Brush: ({mx // visual_size}, {my // visual_size})"
