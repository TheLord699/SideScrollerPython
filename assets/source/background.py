        ...
        try:
            with open(background_file, "r") as file:
                ...
                for bg_id, bg_data in self.bg_settings.items():
                    ...
                    width = bg_data.get("width", 0)
                    height = bg_data.get("height", 0)

                    if image_path and os.path.exists(image_path):
                        image_surface = pg.image.load(image_path)  # no convert/alpha
                        ...
                        image_texture = Texture.from_surface(self.game.renderer, image_surface)
    ...
    def render(self):
        for bg in self.layers:
            ...
            if not repeat_horizontal and not repeat_vertical:
                ...
                bg["texture"].draw(dstrect=(render_x, render_y, bg_width, bg_height))
                ...
                for x in range(start_x, self.game.screen_width, bg_width):
                    for y in range(start_y, self.game.screen_height, bg_height):
                        bg["texture"].draw(dstrect=(x, y, bg_width, bg_height))
                ...
                for x in range(start_x, self.game.screen_width, bg_width):
                    bg["texture"].draw(dstrect=(x, render_y, bg_width, bg_height))
                ...
                for y in range(start_y, self.game.screen_height, bg_height):
                    bg["texture"].draw(dstrect=(render_x, y, bg_width, bg_height))
