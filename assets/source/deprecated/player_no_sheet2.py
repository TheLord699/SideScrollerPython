...
class Player:
    ...
    def render_health(self):
        ...
        for heart in range(self.max_health):
            ...
            if heart + 1 <= self.current_health:
                image_path = "assets/sprites/gui/health/full.png"
                
            elif heart + 1 - self.current_health == 0.5:
                image_path = "assets/sprites/gui/health/half.png"
                
            else:
                image_path = "assets/sprites/gui/health/empty.png"

            self.game.ui.create_ui(
                image_path=image_path, 
                x=x_position, y=y_position, 
                centered=True, width=80, height=80, 
                alpha=True, 
                element_id=heart,
                render_order=1
            )
    ...
    def render_item_info(self, id): # doesnt update amount in real time
        ...
        if not self.selected_slot == None:
            self.game.ui.create_ui(
                image_path=f"assets/sprites/gui/items/{self.inventory[id]["name"]}.png",
                x=150, y=250,
            ...
            self.game.ui.create_ui(
                ...
                render_order=1, font=self.game.environment.fonts["pixel"],
    ...
    def refresh_inventory(self):
        ...
        for slot in range(self.max_inventory_slots):
            ...
            self.game.ui.create_ui(
                image_path=f"assets/sprites/gui/slot.png", 
                x=x_position, y=y_position, 
    ...
    def render_inventory(self):
        if self.in_inventory:
            ...
            for item_slot, item in self.inventory.items():
                ...
                self.game.ui.create_ui(
                    image_path=f"assets/sprites/gui/items/{item['name']}.png", 
                    x=x_position, y=y_position, 
    ...
    def update_collision(self): # absolutely horrendous
        self.hitbox_set()
        ...
        for index, tile_hitbox in enumerate(self.game.map.tile_hitboxes):
            ...
            if self.hitbox.colliderect(tile_hitbox):
                ...
                if overlap_x < overlap_y:
                    horizontal_collisions.append((tile_hitbox, overlap_x, tile_id, swimmable, damage))
                    
                else:
                    vertical_collisions.append((tile_hitbox, overlap_y, tile_id, slippy, damage))

        for tile_hitbox, overlap_x, tile_id, swimmable, damage in horizontal_collisions:
            if swimmable:
                continue
        ...
        for tile_hitbox, overlap_y, tile_id, slippy, damage in vertical_collisions:
    ...
    def handle_gravity(self):
        ...
        for index, tile_hitbox in enumerate(self.game.map.tile_hitboxes): # kinda silly having collision checks in handle_gravity()
            if tile_hitbox.collidepoint(bottom_middle_x, bottom_middle_y):
                ...
                if self.vel_y >= 1.5 and not swimmable:
    ...
    def start_attack(self):
        if not self.attacking:
            ...
            pg.mixer.Sound(f"assets/sounds/attack/swing{attack}.wav").play().set_volume(self.game.environment.volume) # temporary
    ...
    def render(self):
        ...
        sprite_y = self.hitbox.centery - self.cam_y + 1.7 - image.get_height() // 2 # 1.7 cuz feet visually not touching ground
