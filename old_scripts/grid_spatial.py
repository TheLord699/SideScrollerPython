...
class Map:
    ...
    def init_spatial_grid(self):
        ...
        self.gridCellSize = self.visual_tile_size * 4
        self.gridWidth = int((max_x - min_x) // self.gridCellSize) + 1
        self.gridHeight = int((max_y - min_y) // self.gridCellSize) + 1
        self.gridOffsetX = min_x
        self.gridOffsetY = min_y

        self.grid = [[] for _ in range(self.gridWidth * self.gridHeight)]

        for i, hitbox in enumerate(self.tile_hitboxes):
            min_col = int((hitbox.left - self.gridOffsetX) // self.gridCellSize)
            max_col = int((hitbox.right - self.gridOffsetX) // self.gridCellSize)
            min_row = int((hitbox.top - self.gridOffsetY) // self.gridCellSize)
            max_row = int((hitbox.bottom - self.gridOffsetY) // self.gridCellSize)

            min_col = max(0, min_col)
            max_col = min(self.gridWidth - 1, max_col)
            min_row = max(0, min_row)
            max_row = min(self.gridHeight - 1, max_row)

            for row in range(min_row, max_row + 1):
                for col in range(min_col, max_col + 1):
                    self.grid[row * self.gridWidth + col].append(i)

        print(f"Initialized spatial grid: {self.gridWidth}x{self.gridHeight} cells")

    def get_nearby_tiles(self, hitbox, padding=50):
        search_area = hitbox.inflate(padding * 2, padding * 2)
        nearby_tiles = []

        min_col = int((search_area.left - self.gridOffsetX) // self.gridCellSize)
        max_col = int((search_area.right - self.gridOffsetX) // self.gridCellSize)
        min_row = int((search_area.top - self.gridOffsetY) // self.gridCellSize)
        max_row = int((search_area.bottom - self.gridOffsetY) // self.gridCellSize)
        ...
        for row in range(min_row, max_row + 1):
            for col in range(min_col, max_col + 1):
                for tile_idx in self.grid[row * self.gridWidth + col]:
    ...
    def render_debug(self, hitbox=None, padding=15):
        for row in range(self.gridHeight):
            for col in range(self.gridWidth):
                x = col * self.gridCellSize + self.gridOffsetX - self.cam_x
                y = row * self.gridCellSize + self.gridOffsetY - self.cam_y
                pg.draw.rect(self.game.screen, (100, 100, 100, 50), 
                           (x, y, self.gridCellSize, self.gridCellSize), 1)
                
                cell_index = row * self.gridWidth + col
